...
class PGUpdater:
    ...
    def setup(self, win):
        ...
        self.phase_plot_window = win.addPlot(row=1, col=0, title="Phase")
        ...
        for s in range(self.num_sensors):
            ...
            pen_bg = pg.mkPen(0.2, width=2, style=QtCore.Qt.DotLine)
            self.clutter_plot.append(self.envelope_plot_window.plot(range(10),
                                     np.zeros(10), pen=pen_bg, name="Background"))
            ...
            self.phase_plot.append(self.phase_plot_window.plot(range(10),
                                   np.arange(10)*0, pen=pen, name="IQ Phase"))

        title = "Envelope History"
        lut = example_utils.pg_mpl_cmap("viridis")

        self.hist_plot_image = win.addPlot(row=2, col=0, title=title)
    ...
    def update(self, data):
        ...
        for s in range(num_sensors):
            ...
            self.phase_plot[s].setData(data["x_mm"], data["phase"][s])
