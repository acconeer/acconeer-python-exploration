...
def test_init(client_info):
    assert client_info.address == "addr"
    assert client_info.serial_port == "port"
    assert client_info.override_baudrate == 0


def test_eq(client_info):
    assert client_info == ClientInfo(address="addr", serial_port="port", override_baudrate=0)
    assert client_info != ClientInfo(address="ddr", serial_port="port", override_baudrate=0)
