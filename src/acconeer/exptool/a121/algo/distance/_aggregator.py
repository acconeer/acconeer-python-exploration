...
class Aggregator:
    ...
    def process(self, extended_result: list[dict[int, a121.Result]]) -> AggregatorResult:
        ...
        for spec, processor in zip(self.specs, self.processors):
            ...
            if processor_result.estimated_distances is not None:
                rcs = np.concatenate((rcs, np.array(processor_result.estimated_rcs)))
                dists = np.concatenate((dists, np.array(processor_result.estimated_distances)))
        (dists_merged, rcs_merged) = self._merge_peaks(self.MIN_PEAK_DIST_M, dists, rcs)
        (dists_sorted, rcs_sorted) = self._sort_peaks(
            dists_merged, rcs_merged, self.config.peak_sorting_method
        )
    ...
    def _merge_peaks(
        ...
        return (np.array(distances_merged), np.array(rcs_merged))
